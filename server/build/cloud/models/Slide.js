...
// types in here
const POSSIBLE_SEGMENT_TYPES = [IMAGE_TYPE_NAME, // first : Image
    ...
    parseJson() {
        return new Promise((fulfill, reject) => {
            ...
            this.validateId().then(() => {
                return this.validateProjectSlideId();
            }).then(() => {
                return this.validateLayeringObjects();
            }).then(() => {
                return this.validateHyperlinks();
            }).then(() => {
                return this.validateType();
            }).then(() => {
                return this.validateResource();
            }).then(() => {
                return this.validateIsEdited();
            }).then(() => {
                fulfill(this);
            }).catch(error => {
                reject(Error(error));
        ...
        return new Promise((fulfill, reject) => {
            ...
            return new Promise((fulfill, reject) => {
                ...
                for (var i = 0; i < hyperlinks.length; i++) {
                    ...
                    query.get(hyperlinks[i]).then(result => {
                        numHyperlinks += 1;
    ...
    validateResource() {
        return new Promise((fulfill, reject) => {
            ...
            if (this.object[TYPE_FIELD] === IMAGE_TYPE_NAME) {
                new Image(JSON.stringify(resource)).then(result => {
            ...
            if (this.object[TYPE_FIELD] === VIDEO_TYPE_NAME) {
                new Video(JSON.stringify(resource)).then(result => {
            ...
            if (this.object[TYPE_FIELD] === QUESTION_TYPE_NAME) {
                new Question(JSON.stringify(resource)).then(result => {
